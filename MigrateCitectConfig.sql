...
(
       SELECT 
       T1.ID,T1.TagName, T1.Address, T1.DataTypeID, T1.DatasourceID, T1.Location, T1.Properties, T1.LastReadSampleTime, T2.PropertyName, T2.PropertyValue  
       FROM  Tags T1,  TagProperties T2
...
SELECT @TagName = TagName, @Description = Properties, @DataSourceID = DatasourceID, @DataTypeID = DataTypeID,
              @Location = Location, @MaxEu = Eng_Full, @MinEu = Eng_Zero, @EngineeringUnits = Eng_Units, @ValueDeaband = LoggingDeadband,
              @MinRaw = Raw_Zero, @MaxRaw = Raw_Full
FROM #CitectConfig 
 WHERE ID = @TagID   
 
SELECT @DataSource = DatasourceName FROM Datasources WHERE ID = @DataSourceID

IF NOT EXISTS( SELECT 1 FROM Runtime.dbo.EngineeringUnit WHERE Unit = @EngineeringUnits)
BEGIN
	EXEC Runtime.dbo.aaEngineeringUnitInsert   @EngineeringUnits, 1000, 1
END
SELECT @EUKey = EUKey FROM Runtime.dbo.EngineeringUnit WHERE Unit = @EngineeringUnits
...
DECLARE @VDeaband AS FLOAT(53) 
 SET @VDeaband = CONVERT(FLOAT, @ValueDeaband)
  ...
  IF @DataTypeID = 1
  BEGIN
       EXEC Runtime.dbo.aaAnalogTagInsert @TagName, @Description,2,2,0,N'',0,N'Citect',NULL,0,@EUKey, @MnEU,  @MxEU, @MnRaw , @MxRaw ,1,3,@VDeaband,0,16,0,NULL,NULL,1,0,0,0,254,0,0,1,NULL,NULL,0,NULL,1,NULL,NULL
       --EXEC Runtime.dbo.aaAnalogTagInsert @TagName, @Description,1,2,0,N'',0,N'Citect',NULL,0,1,CAST(SELECT @MinEu as float), CAST(SELECT @MaxEu as float),CAST(SELECT @MinRaw as float),CAST(SELECT @MaxRaw as float),1,3,@ValueDeaband,0,16,0,NULL,NULL,1,0,0,0,254,0,0,1,NULL,NULL,0,NULL,1,NULL,NULL
  END

  IF @DataTypeID = 2
  BEGIN
       EXEC Runtime.dbo.aaDiscreteTagInsert @TagName, @Description,2,2,0,N'',0,N'Citect',NULL,0,1,0,NULL,NULL,0,0,0,1,NULL, NULL,0, NULL, 1, NULL, NULL
  END

  IF @DataTypeID = 3
  BEGIN
       EXEC Runtime.dbo.aaStringTagInsert @TagName, @Description,2,2,0,N'',0,N'Citect',NULL,131,N'', NULL, NULL, 0,0, 0,0, 1,0, NULL, NULL, 0, NULL, 1, NULL, NULL , NULL
  END
